...
func NewCache(size int) (*Cache, error) {
	return NewCacheWithEvict(size, nil)
}

// NewWithEvict constructs a fixed size cache with the given eviction callback.
func NewCacheWithEvict(size int, onEvicted func(key interface{}, value interface{})) (*Cache, error) {
	l, err := lru.NewLRU(size, onEvicted)
	if err != nil {
		return nil, err
	}
	c := &Cache{lru: l}
